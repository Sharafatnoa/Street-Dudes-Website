...
const MENU_PRICE_MAP = new Map<string, number>(
  menuCategories.flatMap(cat =>
    cat.items.map(item => [item.id, item.price])
  )
)
...
export async function POST(request: NextRequest) {
  ...
  if (fulfillmentType === 'delivery') {
    if (!deliveryAddress?.trim()) {
      return NextResponse.json(
        { error: 'Leveransadress saknas.' },
        { status: 400 }
      )
    }
    if (!customerPhone?.trim()) {
      return NextResponse.json(
        { error: 'Telefonnummer krävs för leverans.' },
        { status: 400 }
      )
    }
  }
  ...
  if (fulfillmentType === 'delivery') {
    ...
    serverDeliveryFee = config.freeDeliveryThresholdKr > 0
  ...
  if (fulfillmentType === 'delivery') {
    serverDeliveryFee = serverSubtotal >= config.freeDeliveryThresholdKr
      ? 0
      : config.deliveryFeeKr
  }
