      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#111] rounded-t-xl flex flex-col max-h-[90vh] md:max-w-lg md:mx-auto md:left-1/2 md:-translate-x-1/2">
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 flex flex-col gap-6">
          ...
          {item.customization.ingredients.length > 0 && (
            <section>
              ...
              <p className="text-xs text-white/25 mt-2">{t('modal.removalNote')}</p>
            </section>
          )}

          {/* Extras — checkbox rows */}
          {(item.customization.hasSauceAddon || item.customization.addonIds.length > 0) && (
            <section>
              ...
              <div className="flex flex-col gap-2">
                ...
                {item.customization.hasSauceAddon && (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="text-white text-sm">{t('modal.sauceLabel')}</p>
                        <p className="text-white/35 text-xs mt-0.5">{t('modal.sauceNote')}</p>
                      </div>
                    </div>
                    <span className="text-brand-gold text-sm shrink-0 ml-4">+10 kr</span>
                  </label>
                )}
                ...
                {item.customization.addonIds.map((addonId) => {
                  ...
                  const isSelected = selectedAddons.some((a) => a.menuItemId === addonId);
                  return (
                    <label
                      key={addonId}
                      className={`flex items-center justify-between px-4 py-3 rounded-sm border cursor-pointer transition-colors ${
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <input
                          ...
                          className="accent-brand-gold"
                        />
                        <span className="text-white text-sm">{t(addon.nameKey)}</span>
                      </div>
        ...
        <div className="px-5 py-4 border-t border-white/10 bg-[#111] shrink-0">
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="font-display text-2xl text-brand-gold">{totalPrice} kr</span>
          </div>
          <button
